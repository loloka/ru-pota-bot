...
assert.strictEqual(SERVICES.length, 5, 'Should define exactly 5 external services');

const requiredIds = ['oopt_registry', 'oopt_nextgis', 'pota_api', 'pota_next', 'osm_tiles'];
...
console.log('✅ PASS: SERVICES array and definitions are valid (5 services)');
...
// 3. Test single fast check (OSM tiles)
const osmResult = await checkServiceById('osm_tiles');
assert(osmResult, 'OSM check result should exist');
assert.strictEqual(osmResult.id, 'osm_tiles');
assert(typeof osmResult.latency === 'number', 'Latency should be a number');
assert(['up', 'degraded', 'down'].includes(osmResult.status), 'Status must be up, degraded, or down');
console.log(`✅ PASS: checkServiceById('osm_tiles') responded with status=${osmResult.status}, latency=${osmResult.latency}ms`);
...
assert.strictEqual(all1.length, 5, 'checkAllServices should return 5 results');
